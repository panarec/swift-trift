<script lang="ts">
    ...
    $: {
        ...
        if(name){
        name.style.fontSize = nickname.length > 15 ? `${1 - nickname.length / nameFontMultiplier}rem` : `${nameFontSize}rem`
        console.log(name.style)
        }
        if(signature){
        signature.style.fontSize = nickname.length > 15 ? `${1 - nickname.length / 65}rem` : `${nameFontSize}rem`
        signature.style.marginBottom = nickname.length > 15 ? `-${nickname.length / 3}px` : `-11px`
        }
